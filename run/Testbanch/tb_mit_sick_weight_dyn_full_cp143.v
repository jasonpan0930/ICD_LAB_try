`timescale 1ns / 1ps
//
// Same as tb_mit_sick_weight_dyn_full.v but default weights image is
// Pattern/weights_ram_143.hex (from weight/check-point143.txt).
//
...
module tb_mit_sick_weight_dyn_full_cp143();
    ...
    task feed_sample;
        ...
        begin
            ...
            if (o_valid !== 1'b1) begin
                $display("[WEIGHT_CP143] TIMEOUT waiting o_valid sample %0d", sid);
                $finish(1);
            end
        end
    endtask
    ...
    initial begin
        ...
        if (SAMPLE_START < 0 || SAMPLE_START >= TOTAL_SAMPLES) begin
            $display("[WEIGHT_CP143] FATAL: SAMPLE_START(%0d) out of range [0,%0d)", SAMPLE_START, TOTAL_SAMPLES);
            $finish(1);
        end
        ...
        $display("\n[WEIGHT_CP143] weights=`PATTERN_WEIGHTS_HEX; samples %0d .. %0d (%0d total)",
        ...
        for (s = SAMPLE_START; s < TOTAL_SAMPLES; s = s + 1) begin
            ...
            if (o_class !== golden_val)
                $display("[WEIGHT_CP143] NG sample %0d pred=%0d golden=%0d", s, o_class, golden_val);
            else
                $display("[WEIGHT_CP143] OK sample %0d pred=%0d golden=%0d", s, o_class, golden_val);
        ...
        $display("\n=====================================================================");
        $display("[WEIGHT_CP143] summary: correct %0d / %0d (%.4f %%)",
